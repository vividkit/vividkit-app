 ...
 * Ported from claude-devtools/src/renderer/constants/teamColors.ts
 *
...
import type { TeamColorSet } from '@/types/subagent'
...
/** Hash string to number for deterministic color selection */
function hashString(str: string): number {
  let hash = 0
  for (let i = 0; i < str.length; i++) {
    hash = ((hash * 31 + str.charCodeAt(i)) | 0) >>> 0
  }
  return hash
}

/**
 * Get a TeamColorSet for a subagent type.
 * Uses hash-based deterministic color selection.
 */
