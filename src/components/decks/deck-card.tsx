...
export function DeckCard({ deck, onSelect }: DeckCardProps) {
  ...
  const taskCount = tasks.filter((tk) => tk.deckId === deck.id).length
  ...
  async function handleDelete() {
    try {
      ...
      console.error('[DeckCard] delete error', err)
    }
  }
  ...
  return (
    <>
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
            ...
            <Button
              ...
              disabled={isLastDeck}
              title={isLastDeck ? t('pages.decks.actions.cannotDeleteLast') : t('pages.decks.actions.delete')}
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('pages.decks.deleteDialog.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('pages.decks.deleteDialog.description', { name: deck.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
